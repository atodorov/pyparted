...
class DiskGetMaxLogicalPartitionsTestCase(RequiresDisk):
    """getMaxLogicalPartitions return int values based on which type of disk is
    used. It search for occurrence of keys in device path(e.g. /dev/sda) and
    compare it with predefined values("sd": 11). As unittest environment is
    using file with generic name(e.g. /tmp/temp-device-C64w78), test focus
    only on type of returned value
    """
    def runTest(self):
...
class DiskGetMaxSupportedPartitionCountTestCase(RequiresDisk):
    ...
    find value in /sys/block/DEV/ext_range .
    Also see testcase DiskGetMaxSupportedPartitionCountTestCase
    in tests/test__ped_disk , which tests value returned by source C function
    defined in module _ped
    """
    def runTest(self):
        self.assertEqual(self.disk.maxSupportedPartitionCount, 64)

class DiskGetMaxSupportedPartitionCountLoopDiskTestCase(RequiresMount):
    """If value ext_range for device is available, disk.maxSupportedPartitionCount
    should return this value.
    """
    def setUp(self):
        RequiresMount.setUp(self)
        self.mkfs()
        self.doMount()

    def runTest(self):
        path_ext_range = "/sys/block/%s/ext_range" % self.loop.split("/")[2]
        value_ext_range = open(path_ext_range, 'r').read().strip()
        value_ext_range = int(value_ext_range)
